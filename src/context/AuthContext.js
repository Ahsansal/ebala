...
const AuthProvider = ({ children }) => {
  ...
  const handleLogin = (params, errorCallback) => {
    ...
    axios
      ...
      .then(async res => {
        ...
        if (response?.success) {
          window.localStorage.setItem(findAuthConfig?.storageTokenKeyName, response?.token)
  ...
  const handleRegister = (params, errorCallback) => {
    axios
      .put(process.env.NEXT_PUBLIC_BASE_URL + authConfig.admin.registerEndpoint, params)
      .then(res => {
        router.push('/login')
        // if (res.data.error) {
